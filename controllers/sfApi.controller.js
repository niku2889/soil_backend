...
exports.getcrops = (req, res) => {
    ...
    Request.get({
        ...
        "url": "https://test-api.smart-fertilizer.com/crop/" + req.body.userId + "/cropnames?token=" + req.body.token
    }, (error, response, body) => {
        if (error) {
...
exports.getvarieties = (req, res) => {
    ...
    Request.get("https://test-api.smart-fertilizer.com/crop/" + req.body.userId + "/" + req.body.cropId + "?token=" + req.body.token, (error, response, body) => {
        if (error) {
            return res.status(400).send(error);
        }
